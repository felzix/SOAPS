...
public class ViewPatientActivity extends Activity {
    ...
    public void onResume() {
    	...
    	String[] projection = new String[] {
        		...
        		PatientsTable.COLUMN_ENTRY_COUNT
        };
    	
    	Uri uri = Uri.parse(SOAPSContentProvider.CONTENT_URI + "/patients/" + String.valueOf(patient.getId()));
        ...
        projection = new String[] {
        		...
        		FormsTable.COLUMN_PATIENT_ID,
        		FormsTable.COLUMN_DATE
        };
        ...
        if (c != null) {
            if (c.moveToFirst()) {
                do {
                	long patient_id = c.getLong(c.getColumnIndex(FormsTable.COLUMN_PATIENT_ID));
                	if (patient_id == patient.getId()) {
                		long id = c.getLong(c.getColumnIndex(FormsTable.COLUMN_ID));
                        String date_str = c.getString(c.getColumnIndex(FormsTable.COLUMN_DATE));
        ...
        ArrayAdapter<Form> adapter = new ArrayAdapter<Form>(this, android.R.layout.simple_list_item_1, forms);
        lv.setAdapter(adapter);
        lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
        	@Override
        	public void onItemClick(AdapterView<?> adapter, View view, int position, long l) {
        		Intent intent = new Intent(ViewPatientActivity.this, ViewFormActivity.class);
